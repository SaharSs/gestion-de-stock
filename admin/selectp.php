...
if(isset($_POST['recherche']) AND !empty($_POST['recherche'])) {
 ...
 if(isset($_GET['page']))
	...
	$page=1;
     ...
     $q5 = "SELECT * FROM produit WHERE name LIKE '%".$m."%'  limit $depart, $num_an  ";
  ...
  <tbody>
    <tr>
      ...
      <td><?php echo $row['qte'] ?></td>
         ...
         <td>
          <a href="updatep.php?id=<?=$row['id'];?>"><i class="fas fa-edit"></i></a>
		</td>
		<td>
          <a  onclick="return confirm('etes vous sur');"   href="deletep.php?id=<?=$row['id'];?>"><i class="fas fa-trash"></i></a>
...
$q1="select id from produit WHERE name LIKE '%".$m."%'";
...
if(isset($_GET['page']))
    ...
    $q3 = "SELECT * FROM produit WHERE name LIKE '%".$l."%'  limit $depart, $num_an  ";
  ...
  <tbody>
    <tr>
      ...
      <td><?php echo $row1['qte'] ?></td>
         <td>
          <a href="updatep.php?id=<?=$row1['id'];?>"><i class="fas fa-edit"></i></a>
		</td>
		<td>
          <a  onclick="return confirm('etes vous sur');"   href="deletep.php?id=<?=$row1['id'];?>"><i class="fas fa-trash"></i></a>
...
$q1="select id from produit WHERE name LIKE '%".$l."%'";
$r2=mysqli_query($c,$q1);
$annonce_t=mysqli_num_rows($r2);	
$pagesTotales = ceil($annonce_t/$num_an);
	?>
		...
		}else
{
	...
	$depart=($page-1)*	$num_an;
    $q1 = "SELECT * from produit    limit $depart, $num_an ";
...
?>
  ...
  <?php
 $q1="select id from produit";
 $r2=mysqli_query($c,$q1);
 $annonce_t=mysqli_num_rows($r2);	
 $pagesTotales = ceil($annonce_t/$num_an);
   if($e<=10)
   {
   echo "<script type='text/javascript'>alert('Low stock restock it immedietly');</script>";	
   }
?>
      <nav aria-label="Page navigation example">
      <ul class="pagination">
